...
const ADD_SALES_LINE_MESSAGES: Record<string, string> = {
  ...
  claim_lost: 'Edit session expired — tap Done and open Edit again.',
  order_not_found: 'Order could not be found.',
  not_submitted: 'This order is past the submitted stage and cannot be edited here.',
  ...
  submit_failed: 'Could not add this line. Try again.',
};

type SalesEditAddLineSheetProps = {
  isOpen: boolean;
  onClose: () => void;
  orderId: number;
...
export function SalesEditAddLineSheet({
  isOpen,
  onClose,
  orderId,
  stockLocationCode,
  ...
  existingItems,
  onAdded,
}: SalesEditAddLineSheetProps): ReactElement | null {
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      if (!selected || !claimId || !userId) throw new Error('Missing selection or edit lock');
        ...
        const code =
          typeof errField === 'string'
            ...
            : errField != null
              ...
              : 'add_failed';
        const base = ADD_SALES_LINE_MESSAGES[code] ?? code;
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="sales-add-line-title"
      >
        <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-[var(--border-subtle)] shrink-0">
          <h2 id="sales-add-line-title" className="text-base font-bold text-[var(--content-primary)]">
            Add line
          </h2>
          <button
            type="button"
