-- orders.item_count was incorrectly storing sum of shippable qty (Busy cares about invoice line count).
-- Store unique order_items line count; backfill from order_items.
...
COMMENT ON FUNCTION public.submit_sales_order(jsonb) IS
  'Sales checkout: FOR UPDATE items, decrement stock by shippable qty, insert order + lines + pending PO rows atomically. orders.item_count = number of order lines (invoice lines).';

-- Fix existing rows: item_count = count(order_items), not sum(qty).
