-- Handoff summary v2: workflow roles (claimed → assigned → picked → resolved → completed) + timestamps.
...
BEGIN
  ...
  WHERE oe.order_id = p_order_id
    AND oe.event_type = 'billing_claimed'
  ...
  WHERE oe.order_id = p_order_id
    ...
    AND NULLIF(TRIM(oe.payload->>'via'), '') IS DISTINCT FROM 'billing_force_complete_pre_pick'
  ORDER BY oe.created_at DESC
  LIMIT 1;
  ...
  LEFT JOIN public.users u ON u.id = oe.actor_user_id
  WHERE oe.order_id = p_order_id
    AND oe.event_type IN ('billing_flags_resolved', 'billing_approved')
    AND (
      oe.event_type = 'billing_flags_resolved'
      OR v_order.workflow_status = 'completed'
    )
  ORDER BY
    CASE oe.event_type WHEN 'billing_flags_resolved' THEN 0 ELSE 1 END,
    oe.created_at DESC
  LIMIT 1;

  IF v_completed_by IS NULL AND v_order.workflow_status = 'completed' THEN
    v_completed_by := NULLIF(TRIM(v_order.reviewer_name), '');
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'submittedAt', v_order.created_at,
...
COMMENT ON FUNCTION public.get_order_handoff_summary(BIGINT) IS
  'Workflow handoff: claimed, assigned, picked, resolved, completed roles plus submit-to-done duration.';
