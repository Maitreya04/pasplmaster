-- Revert sales selling units (108–110): restore pre-108 RPCs, drop columns and helper.
-- Safe to run even if 108 never applied (uses IF EXISTS / CREATE OR REPLACE).
-- Run on production if migrations 108–110 were applied during the kit/set/nos rollout.

DROP FUNCTION IF EXISTS public.add_sales_submitted_line(
  BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT, BOOLEAN, TEXT
);

DROP FUNCTION IF EXISTS public.sales_unit_ea_multiplier(JSONB, TEXT);

ALTER TABLE public.order_items
  DROP COLUMN IF EXISTS sales_selling_unit;

ALTER TABLE public.items
  DROP COLUMN IF EXISTS sales_selling_units;
