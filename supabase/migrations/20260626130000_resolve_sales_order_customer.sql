-- Offline replay may carry stale customer ids after ERP re-imports.
-- Resolve by id first, then fall back to normalized customer name.

-- Resolve stale offline customer ids by id first, then normalized name.
...
BEGIN
  ...
  INTO v_id, customer_name, customer_city
...
BEGIN
  BEGIN
    ...
    SELECT r.customer_id, r.customer_name, r.customer_city
    INTO v_customer_id, v_customer_name, v_customer_city
    FROM public.resolve_sales_order_customer(v_customer_id, v_customer_name, v_customer_city) AS r
    LIMIT 1;
