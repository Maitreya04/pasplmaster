-- submit_sales_order kept the payload customer_id when resolve_sales_order_customer
-- returned no rows (SELECT INTO does not clear targets). Guard with FOUND + reset.

CREATE OR REPLACE FUNCTION public.normalize_sales_order_customer_name(p_name TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
...
BEGIN
  ...
  v_normalized_name := public.normalize_sales_order_customer_name(v_name);
  ...
  WHERE c.is_active = true
    AND public.normalize_sales_order_customer_name(c.name) = v_normalized_name
  ORDER BY c.id
  LIMIT 1;
...
GRANT EXECUTE ON FUNCTION public.normalize_sales_order_customer_name(TEXT) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.resolve_sales_order_customer(BIGINT, TEXT, TEXT) TO anon, authenticated, service_role;

-- Fix submit_sales_order customer resolution guard.
...
BEGIN
  BEGIN
    ...
    v_payload_customer_id := NULLIF(TRIM(p_payload->>'customer_id'), '')::BIGINT;
    v_customer_name := NULLIF(TRIM(p_payload->>'customer_name'), '');
    IF v_payload_customer_id IS NULL OR v_customer_name IS NULL OR length(trim(v_customer_name)) = 0 THEN
    ...
    INTO v_customer_id, v_customer_name, v_customer_city
    FROM public.resolve_sales_order_customer(v_payload_customer_id, v_customer_name, v_customer_city)
    LIMIT 1;
    ...
    IF NOT FOUND OR v_customer_id IS NULL THEN
      RETURN jsonb_build_object(
        ...
        'detail', 'Customer was not found. Refresh your customer list and try again.'
      );
    END IF;
