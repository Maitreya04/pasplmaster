-- Free-of-charge (FOC) lines: persist explicit flag alongside zero quoted price.

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS is_foc BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN public.order_items.is_foc IS
  'True when this line is billed at zero as manual/system free qty (FOC). Distinct from incidental zero quotes.';

-- Replace submit_sales_order: honour payload is_foc and keep price_quoted at 0 for FOC rows.
...
COMMENT ON FUNCTION public.submit_sales_order(jsonb) IS
  'Sales checkout: splits by guarded location-wise availability minus reservations; creates active stock_reservations; supports FOC lines (is_foc).';
...
DROP FUNCTION IF EXISTS public.add_billing_line(BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT);
...
COMMENT ON FUNCTION public.add_billing_line(BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT, BOOLEAN) IS
  'Billing live queue: add one order line with allocation + reservation + optional PO pending row. Requires active billing claim. Optional FOC (zero billed).';

GRANT EXECUTE ON FUNCTION public.add_billing_line(BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT, BOOLEAN)
  TO anon, authenticated, service_role;

DROP FUNCTION IF EXISTS public.add_sales_submitted_line(BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT);
...
COMMENT ON FUNCTION public.add_sales_submitted_line(BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT, BOOLEAN) IS
  'Sales edit on submitted order: add line with allocation + reservation + optional PO pending row. Requires active sales_edit claim. Optional FOC.';

GRANT EXECUTE ON FUNCTION public.add_sales_submitted_line(BIGINT, BIGINT, INTEGER, NUMERIC, BIGINT, BIGINT, BOOLEAN)
  TO anon, authenticated, service_role;
