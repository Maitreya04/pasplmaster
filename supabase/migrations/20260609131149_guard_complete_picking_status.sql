...
COMMENT ON FUNCTION public.complete_picking(BIGINT, BIGINT, BIGINT, BOOLEAN, INTEGER) IS
  'Completes an active picking claim only while the order is still in picking; rejects stale or already-finalised sessions.';
...
BEGIN
  ...
  IF v_oi.workflow_status IS DISTINCT FROM 'picking' THEN
    RETURN jsonb_build_object(
      'success', false,
      'reason', 'order_not_picking',
      'status', v_oi.workflow_status
    );
  END IF;
...
GRANT EXECUTE ON FUNCTION public.wms_consume_bin_layer_for_pick(BIGINT, INTEGER, BIGINT, BIGINT, TEXT, TEXT, BIGINT)
  TO anon, authenticated;

COMMENT ON FUNCTION public.wms_consume_bin_layer_for_pick(BIGINT, INTEGER, BIGINT, BIGINT, TEXT, TEXT, BIGINT) IS
  'Consumes picker shelf layers only while the order is still in picking.';
