-- Sales-home framing: period expected = full period target (close the number),
-- not pace-to-date. Achievement stays period-to-date through as_of.
-- Month: full July target vs MTD billed.
-- Quarter: full Q2 target vs QTD billed.
-- FY: full-year target vs YTD billed.
...
DECLARE
  ...
  v_month_end DATE := (date_trunc('month', v_as_of) + interval '1 month - 1 day')::date;
  -- Calendar quarters align with Indian FY quarters when FY starts in April.
  v_quarter_start DATE := date_trunc('quarter', v_as_of)::date;
  v_quarter_end DATE := (date_trunc('quarter', v_as_of) + interval '3 months - 1 day')::date;
  ...
  v_month_workdays INTEGER := 0;
  v_quarter_workdays INTEGER := 0;
  v_remaining_month_workdays INTEGER := 0;
  v_remaining_quarter_workdays INTEGER := 0;
...
BEGIN
  ...
  SELECT
    ...
    count(*) FILTER (
      WHERE is_working_day
        AND calendar_date >= v_month_start
        AND calendar_date <= v_as_of
    ),
    count(*) FILTER (
      WHERE is_working_day
        AND calendar_date >= v_quarter_start
        AND calendar_date <= v_as_of
    ),
    count(*) FILTER (
      WHERE is_working_day
        AND calendar_date >= v_month_start
        AND calendar_date <= least(v_month_end, v_fy.ends_on)
    ),
    count(*) FILTER (
      WHERE is_working_day
        AND calendar_date >= v_quarter_start
        AND calendar_date <= least(v_quarter_end, v_fy.ends_on)
    ),
    count(*) FILTER (
      WHERE is_working_day
        AND calendar_date > v_as_of
  ...
  INTO v_total_workdays, v_elapsed_fy_workdays, v_elapsed_month_workdays,
       v_elapsed_quarter_workdays, v_month_workdays, v_quarter_workdays,
       v_remaining_month_workdays, v_remaining_quarter_workdays,
  ...
  SELECT
    ...
    coalesce(sum(net_achievement) FILTER (
      WHERE sales_date >= v_month_start AND sales_date <= v_as_of
    ), 0),
    coalesce(sum(net_achievement) FILTER (
      WHERE sales_date >= v_quarter_start AND sales_date <= v_as_of
    ), 0),
    coalesce(sum(net_achievement) FILTER (WHERE sales_date <= v_as_of), 0)
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    'month_expected', CASE WHEN v_total_workdays > 0
      THEN c.annual_target * v_month_workdays / v_total_workdays ELSE 0 END,
    'quarter_actual', c.quarter_actual,
    'quarter_expected', CASE WHEN v_total_workdays > 0
      THEN c.annual_target * v_quarter_workdays / v_total_workdays ELSE 0 END,
    'fy_actual', c.fy_actual,
    'fy_expected', c.annual_target,
  ...
  RETURN jsonb_build_object(
    ...
    'working_days', jsonb_build_object(
      ...
      'month', v_month_workdays,
      'quarter', v_quarter_workdays,
      'remaining_month', v_remaining_month_workdays,
      'remaining_quarter', v_remaining_quarter_workdays,
      'remaining', v_remaining_workdays,
      'is_today_working_day', v_today_workday = 1
    ),
    'period_windows', jsonb_build_object(
      'month', jsonb_build_object('starts_on', v_month_start, 'ends_on', least(v_month_end, v_fy.ends_on)),
      'quarter', jsonb_build_object('starts_on', v_quarter_start, 'ends_on', least(v_quarter_end, v_fy.ends_on)),
      'fy', jsonb_build_object('starts_on', v_fy.starts_on, 'ends_on', v_fy.ends_on)
    ),
    ...
    'periods', jsonb_build_object(
      ...
      'month', jsonb_build_object(
        ...
        'expected', CASE WHEN v_total_workdays > 0 THEN v_annual_target * v_month_workdays / v_total_workdays ELSE 0 END
      ),
      'quarter', jsonb_build_object(
        'actual', v_quarter_actual,
        'expected', CASE WHEN v_total_workdays > 0 THEN v_annual_target * v_quarter_workdays / v_total_workdays ELSE 0 END
      ),
      'fy', jsonb_build_object(
        'actual', v_fy_actual,
        'expected', v_annual_target
      )
    ),
