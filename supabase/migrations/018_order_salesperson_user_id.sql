-- Route sales notifications by stable users.id instead of fuzzy name matching.
-- This fixes cross-device leaks when salesperson display names change.

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS salesperson_user_id BIGINT REFERENCES public.users(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_orders_salesperson_user
  ON public.orders(salesperson_user_id);

CREATE OR REPLACE FUNCTION public.normalize_salesperson_key(v text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $func$
DECLARE
  normalized text;
BEGIN
  normalized := lower(regexp_replace(coalesce(trim(v), ''), '[^a-z0-9]+', '', 'g'));
  IF normalized = 'rajuji' THEN RETURN 'raju'; END IF;
  IF normalized = 'asadkhan' THEN RETURN 'asad'; END IF;
  IF normalized = 'manishsharma' THEN RETURN 'manish'; END IF;
  IF normalized = 'hardeepsingh' THEN RETURN 'hardeep'; END IF;
  IF normalized = 'anandawasthi' THEN RETURN 'awasthi'; END IF;
  RETURN normalized;
END;
$func$;
...
COMMENT ON FUNCTION public.submit_sales_order(jsonb) IS
  'Sales checkout: stores both salesperson_name and salesperson_user_id so routing and ownership can use a stable users.id.';
