-- PASPL Master — unify Busy→Supabase stock: optional per-location payload on
-- apply_erp_items_delta so items.stock_qty and stock_locationwise are written
-- from the same snapshot (same RPC transaction). Existing callers that omit
-- `locations` behave unchanged.
...
BEGIN
  ...
  -- Replace all app-facing warehouse rows for this SKU so SUM(view) never double-counts
  -- legacy pairs like "Main Store" + "indore" (both main_store).
  del_sl AS (
...
COMMENT ON FUNCTION public.apply_erp_items_delta(jsonb, text, jsonb) IS
  'Apply ERP/MSSQL deltas keyed by items.busy_code. Optional `locations` array applies stock_locationwise + sets items.stock_qty to the sum in one transaction. Service role only.';
